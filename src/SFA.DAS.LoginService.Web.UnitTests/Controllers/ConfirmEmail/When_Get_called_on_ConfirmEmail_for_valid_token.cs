...
{
    public class When_Get_called_on_ConfirmEmail_for_valid_token : ConfirmEmailTestBase
    {
        [SetUp]
        public void Arrange()
        {
            Mediator.Send(Arg.Any<VerifyConfirmEmailRequest>()).Returns(new VerifyConfirmEmailResponse() { VerifyConfirmedEmailResult = VerifyConfirmedEmailResult.TokenVerified, Email = "email@email.com"});
        ...
        public async Task Then_ViewResult_contains_View_ConfirmEmailSuccessful()
        {
            ...
            result.As<ViewResult>().ViewName.Should().Be("ConfirmEmailSuccessful");
        }
