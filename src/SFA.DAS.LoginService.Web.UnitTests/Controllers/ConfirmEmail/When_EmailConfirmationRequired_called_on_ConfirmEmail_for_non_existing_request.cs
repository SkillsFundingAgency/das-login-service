...
{
    public class When_EmailConfirmationRequired_called_on_ConfirmEmail_for_non_existing_request : ConfirmEmailTestBase
    {
        [SetUp]
        public void Arrange()
        {
            Mediator.Send(Arg.Is<CheckExistsConfirmEmailRequest>(r => r.Email == "test@test.com")).Returns(new CheckExistsConfirmEmailResponse() { HasRequest = false });
        }
        ...
        [Test]
        public async Task Then_ViewResult_contains_View_ConfirmEmailRequired()
        {
            var result = await Controller.EmailConfirmationRequired("test@test.com", Uri.UnescapeDataString(ReturnUrl));

            result.As<ViewResult>().ViewName.Should().Be("ConfirmEmailRequired");
        }
