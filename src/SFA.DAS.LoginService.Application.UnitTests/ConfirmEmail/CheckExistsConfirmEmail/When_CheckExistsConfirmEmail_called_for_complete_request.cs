...
{
    public class When_CheckExistsConfirmEmail_called_for_complete_request : CheckExistsConfirmEmailTestBase
    {
        [SetUp]
        ...
        {
            var result = await Handler.Handle(new CheckExistsConfirmEmailRequest() { Email = "email+two@emailaddress.com" }, CancellationToken.None);
