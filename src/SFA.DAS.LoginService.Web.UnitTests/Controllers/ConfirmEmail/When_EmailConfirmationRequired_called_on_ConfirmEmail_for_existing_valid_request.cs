...
{
    public class When_EmailConfirmationRequired_called_on_ConfirmEmail_for_existing_valid_request : ConfirmEmailTestBase
    {
        [SetUp]
        public void Arrange()
        {
        ...
        [Test]
        public async Task Then_ViewResult_contains_View_ConfirmEmailPendingLink()
        {
            var result = await Controller.EmailConfirmationRequired("test@test.com", Uri.UnescapeDataString(ReturnUrl));

            result.As<ViewResult>().ViewName.Should().Be("ConfirmEmailPendingLink");
        }
